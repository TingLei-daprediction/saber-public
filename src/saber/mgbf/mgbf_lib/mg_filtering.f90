...
!clt      call this%filtering_lin2
   case(5)
! production fast path: Jim's wbfil calibrated filters + new operator order
! (the older filtering_fast_bkg / _new_jim / _new_jim_new_order transitional
! implementations and their mg_filt=15/25 cases have been removed)
      call this%filtering_fast_bkg_new_jim_new_order_wbfil
   case(35)
! deprecated alias of case(5), kept for existing configurations; to be removed
      call this%filtering_fast_bkg_new_jim_new_order_wbfil
   case(6)
...
! Production fast filtering path (mg_filt=5; deprecated alias 35).
! Horizontal and vertical beta filters use the *_jim_new_wbfil
! exact-calibration coefficients (paspx4d/paspy4d/pasp1_jim_new_wbfil);
! boundary reflection uses the rflip*_jim_new routines. The transitional
! filtering_fast_bkg / _new_jim / _new_jim_new_order implementations that
! this path evolved from have been removed.
!***********************************************************************
!                                                                      !
...
real(r_kind):: xLb_jim_x(2),xmb_jim_x(2), &
               xLb_jim_y(2),xmb_jim_y(2)
include "type_parameter_locpointer.inc"
include "type_intstat_locpointer.inc"
include "type_parameter_point2this.inc"
include "type_intstat_point2this.inc"
!-----------------------------------------------------------------------
! boundary reflection scales: precomputed once in def_mg_weights from the
! domain-average raw aspects (same arithmetic as the former in-line sums)
xLb_jim_x=this%xLb_wbfil_x
xmb_jim_x=this%xmb_wbfil_x
xLb_jim_y=this%xLb_wbfil_y
xmb_jim_y=this%xmb_wbfil_y
!***
!*** Adjoint interpolate and upsend 
!***
                                                 call btim(upsend_tim)
...
!**********************************************************************
!                                                                     *
!     vertical wbfil filter for the fast-bkg path:                     *
