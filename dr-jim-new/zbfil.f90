...
!======================================================================[rcalib]
subroutine rcalib1(hx,Lx,mx,Lbx,mbx,as,xLb,xmb,el,hxm)!                            
!==============================================================================
...
! The subdomain spans lx to mx. When halos are added, the extended
! subdomain goes from kx=Lx-hx to nx=mx+hx, where hx is at least as large as
! the half-span of any beta filter applied to this grid. hx is called the
! "halo width".
! When subdomain boundary point Lx is also the boundary of the whole domain,
! and if the remediating cosmetic boundary treatment is desired to be used
! at this boundary, this fact must be indicated by setting the logical flag
! Lbx to .true. but if Lx is merely an interior point, or it is not desired
! to apply the remediating boundary condition, Lbx is input as .true. here,
! and in the filtering (rbetat, rbeta) and flipt and flip routines.
! Likewise, when the subdomain boundary point mx is also a domain boundary
! point, and remedial boundary treatment is desired here, then logical flag
! mbx is set to .true. for calibration (here) and in the filtering and flip
! routines for this data tile of the decomposition. The real values xLb and
! xmb are the inverse correlation scales, in grid units, normal to the
! boundaries in the case where, respectively, Lbx or mbx are .true., and
! are simply initialized here from the prescribed aspect tensor field, AS,
! at those boundary points for later use in the flipt and flip routines.
! These boundary interventions to reduce the magnitude of filter response
! distortion at boundaries are taken into account here when the normalizing
! coefficient of the filter is derived (and placed for in array el(0,:) for
! later use in rbetat and rbeta).
!==============================================================================
use pkind, only: dp,spi
...
real(dp)                  :: b,exx,f,r,rc,rrc,s,x,rx
integer(spi)              :: ib,ix,ixp,ixm,gx,gxm,gxn,Lxmix,mxmix
!==============================================================================
if(lbx)xLb=u1/sqrt(as(Lx))! Lx-boundary, inverse filter spread
if(mbx)xmb=u1/sqrt(as(mx))! Mx-boundary, inverse filter spread 
do ix=Lx,mx
   b=sqrt(as(ix))
   ...
   if(Lbx)then
      ...
      gxn=gxm+Lxmix
      do gx=1,gxn
         rx=gx*xLb; rx=om0+rx*(om1+rx*om2)
         ixm=lxmix-gx
         ixp=lxmix+gx-1
         fs(ixp)=fs(ixp)+rx*fs(ixm); fs(ixm)=0
      enddo
   endif
   if(mbx)then
      mxmix=mx-ix
      gxn=gxm-mxmix
      do gx=1,gxn
         rx=gx*xmb; rx=om0+rx*(om1+rx*om2)
         ixp=mxmix+gx
         ixm=mxmix-gx+1
         fs(ixm)=fs(ixm)+rx*fs(ixp); fs(ixp)=0
      enddo
   endif
...
!===================================================================== [rcalib]
subroutine rcalib2(hx,lx,mx, hy,ly,my, lbx,mbx,lby,mby,as,&
...
real(dp),dimension(3  ,-hx+Lx:mx+hx,-hy+Ly:my+hy),intent(in   ):: as
real(dp),dimension(                 -hy+Ly:my+hy),intent(  out):: xLb,xmb
real(dp),dimension(    -hx+Lx:mx+hx             ),intent(  out):: yLb,ymb
...
real(dp),dimension(       -hy:hy):: rx
real(dp),dimension(-hx:hx       ):: ry
real(dp),dimension(2,2)          :: tas,tel22
real(dp),dimension(3)            :: tel
real(dp)                         :: aa0,cx,exx,eyy,eyx,f,r,rrc,rrxc
integer(spi)                     :: gx,gxmm,gxL,gxm,gy,gym,gxn,gyn,&
     ix,ixm,ixp,iy,iym,iyp, kx,nx,Lxm,mxp,kxr,nxr, ky,ny,Lym,myp,kyr,nyr,&
     Lxmix,mxmix,Lymiy,mymiy,p2p4
!==============================================================================
p2p4=p*2+4
if(Lbx)then! "West":
   Lym=Ly-1; myp=my+1; ky=Ly-hy; ny=my+hy; kyr=lym+hy; nyr=myp-hy
   xLb(Ly:my)=u1/sqrt(as(1,Lx,Ly:my))
   if(Lby)then; xLb(ky:Lym)=xLb(kyr:Ly:-1)
   else       ; xLb(ky:Lym)=u1/sqrt(as(1,Lx,ky:Lym))
   endif
   if(mby)then; xLb(myp:ny)=xLb(my:nyr:-1)
   else       ; xlb(myp:ny)=u1/sqrt(as(1,Lx,myp:ny))
   endif
endif
if(mbx)then! "East":
   Lym=Ly-1; myp=my+1; ky=Ly-hy; ny=my+hy; kyr=lym+hy; nyr=myp-hy
   xmb(Ly:my)=u1/sqrt(as(1,mx,Ly:my))
   if(Lby)then; xmb(ky:Lym)=xmb(kyr:Ly:-1)
   else       ; xmb(ky:Lym)=u1/sqrt(as(1,mx,ky:Lym))
   endif
   if(mby)then; xmb(myp:ny)=xmb(my:nyr:-1)
   else       ; xmb(myp:ny)=u1/sqrt(as(1,mx,myp:ny))
   endif
endif
if(Lby)then! "South":
   Lxm=Lx-1; mxp=mx+1; kx=Lx-hx; nx=mx+hx; kxr=lxm+hx; nxr=mxp-hx
   yLb(Lx:mx)=u1/sqrt(as(2,Lx:mx,Ly))
   if(Lbx)then; yLb(kx:Lxm)=yLb(kxr:Lx:-1)
   else       ; yLb(kx:Lxm)=u1/sqrt(as(2,kx:Lxm,Ly))
   endif
   if(mbx)then; yLb(mxp:nx)=ymb(mx:nxr:-1)
   else       ; yLb(mxp:nx)=u1/sqrt(as(2,mxp:nx,Ly))
   endif
endif
if(mby)then! "North":
   Lxm=Lx-1; mxp=mx+1; kx=Lx-hx; nx=mx+hx; kxr=lxm+hx; nxr=mxp-hx
   ymb(Lx:mx)=u1/sqrt(as(2,Lx:mx,my))
   if(Lbx)then; ymb(kx:Lxm)=ymb(kxr:Lx:-1)
   else       ; ymb(kx:Lxm)=u1/sqrt(as(2,kx:Lxm,my))
   endif
   if(mbx)then; ymb(mxp:nx)=ymb(mx:nxr:-1)
   else       ; ymb(mxp:nx)=u1/sqrt(as(2,mxp:nx,my))
   endif
endif
...
! If applicable, treat domain boundary conditions   
   if(Lbx)then
      ...
      gxn=gxm+Lxmix
      do gx=1,gxn
         rx(-gym:gym)=gx*xLb(iy-gym:iy+gym)
         do gy=-gym,gym; rx(gy)=om0+rx(gy)*(om1+rx(gy)*om2)
         enddo
         ixm=lxmix-gx
         ixp=lxmix+gx-1
         fs(ixp,-gym:gym)=fs(ixp,-gym:gym)+rx(-gym:gym)*fs(ixm,-gym:gym); fs(ixm,-gym:gym)=0
      enddo
   endif
   if(mbx)then
      mxmix=mx-ix
      gxn=gxm-mxmix
      do gx=1,gxn
         rx(-gym:gym)=gx*xmb(iy-gym:iy+gym)
         do gy=-gym,gym; rx(gy)=om0+rx(gy)*(om1+rx(gy)*om2)
         enddo
         ixp=mxmix+gx
         ixm=mxmix-gx+1
         fs(ixm,-gym:gym)=fs(ixm,-gym:gym)+rx(-gym:gym)*fs(ixp,-gym:gym); fs(ixp,-gym:gym)=0
      enddo
   endif
   ...
   if(Lby)then
      ...
      gyn=gym+Lymiy
      do gy=1,gyn
         ry(-gxm:gxm)=gy*yLb(ix-gxm:ix+gxm)
         do gx=-gxm,gxm; ry(gx)=om0+ry(gx)*(om1+ry(gx)*om2)
         enddo
         iym=lymiy-gy
         iyp=lymiy+gy-1
         fs(-gxm:gxm,iyp)=fs(-gxm:gxm,iyp)+ry(-gxm:gxm)*fs(-gxm:gxm,iym); fs(-gxm:gxm,iym)=0
      enddo
   endif
   if(mby)then
      mymiy=my-iy
      gyn=gym-mymiy
      do gy=1,gyn
         ry(-gxm:gxm)=gy*ymb(ix-gxm:ix+gxm)
         do gx=-gxm,gxm; ry(gx)=om0+ry(gx)*(om1+ry(gx)*om2)
         enddo
         iyp=mymiy+gy
         iym=mymiy-gy+1
         fs(-gxm:gxm,iym)=fs(-gxm:gxm,iym)+ry(-gxm:gxm)*fs(-gxm:gxm,iyp); fs(-gxm:gxm,iyp)=0
      enddo
   endif
...
!===================================================================== [rcalib]
subroutine rcalib3(hx,Lx,mx, hy,Ly,my, hz,Lz,mz, Lbx,mbx,Lby,mby,Lbz,mbz,&
     as, xLb,xmb,yLb,ymb,zLb,zmb,el,hxm,hym,hzm)
!==============================================================================
! Convert the given field, as, of aspect tensors into the equivalent field el(1:,..)
! of Cholesky lower-triangular factors of the inverses of the aspect tensors
...
real(dp),dimension(6,-hx+Lx:mx+hx,-hy+Ly:my+hy,-hz+Lz:mz+hz),intent(in ):: as
real(dp),dimension(               -hy+Ly:my+hy,-hz+Lz:mz+hz),intent(out):: xLb,xmb
real(dp),dimension(  -hx+Lx:mx+hx,             -hz+Lz:mz+hz),intent(out):: yLb,ymb
real(dp),dimension(  -hx+Lx:mx+hx,-hy+Ly:my+hy             ),intent(out):: zLb,zmb
real(dp),dimension(0:6,  Lx:mx,       Ly:my,       Lz:mz   ),intent(out):: el
integer(spi),dimension(  Lx:mx,       Ly:my,       Lz:mz   ),intent(out):: hxm,hym,hzm
!------------------------------------------------------------------------------
real(dp),dimension(-hx:hx,-hy:hy,-hz:hz):: fs
real(dp),dimension(       -hy:hy,-hz:hz):: rx
real(dp),dimension(-hx:hx,       -hz:hz):: ry
real(dp),dimension(-hx:hx,-hy:hy       ):: rz
...
integer(spi)           :: gx,gxL,gxm,gxn,gy,gyL,gym,gyn,gz,gzm,gzn,gxmm,gymm,&
     ...
     kx,nx,Lxm,mxp,kxr,nxr, ky,ny,Lym,myp,kyr,nyr, kz,nz,Lzm,mzp,kzr,nzr,&
     Lxmix,mxmix,Lymiy,mymiy,Lzmiz,mzmiz,p2p5
!==============================================================================
p2p5=p*2+5
if(Lbx)then! "West":
   Lym=Ly-1; myp=my+1; ky=Ly-hy; ny=my+hy; kyr=Lym+hy; nyr=myp-hy
   Lzm=Lz-1; mzp=mz+1; kz=Lz-hz; nz=mz+hz; kzr=Lzm+hz; nzr=mzp-hz
   xLb(Ly:my,Lz:mz)=u1/sqrt(as(1,Lx,Ly:my,Lz:mz))
   if(Lby)then; xLb(ky:Lym,Lz:mz)=xLb(kyr:Ly:-1,Lz:mz)
   else       ; xLb(ky:Lym,Lz:mz)=u1/sqrt(as(1,Lx,ky:Lym,Lz:mz))
   endif
   if(mby)then; xLb(myp:ny,Lz:mz)=xLb(my:nyr:-1,Lz:mz)
   else       ; xlb(myp:ny,Lz:mz)=u1/sqrt(as(1,Lx,myp:ny,Lz:mz))
   endif
   if(Lbz)then; xLb(Ly:my,kz:Lzm)=xLb(Ly:my,kzr:Lz:-1)
   else       ; xLb(Ly:my,kz:Lzm)=u1/sqrt(as(1,Lx,Ly:my,kz:Lzm))
   endif
   if(mbz)then; xLb(Ly:my,mzp:nz)=xLb(Ly:my,mz:nzr:-1)
   else       ; xlb(Ly:my,mzp:nz)=u1/sqrt(as(1,Lx,Ly:my,mzp:nz))
   endif
   if(Lby)then    ; xLb(ky:Lym,kz:Lzm)=xLb(kyr:Ly:-1,kz:Lzm)
   elseif(Lbz)then; xLb(ky:Lym,kz:Lzm)=xLb(ky:Lym,kzr:Lz:-1)
   else           ; xLb(ky:Lym,kz:Lzm)=u1/sqrt(as(1,Lx,ky:Lym,kz:Lzm))
   endif
   if(Lby)then    ; xLb(ky:Lym,mzp:nz)=xLb(kyr:Ly:-1,mzp:nz)
   elseif(mbz)then; xLb(ky:Lym,mzp:nz)=xLb(ky:Lym,mz:nzr:-1)
   else           ; xLb(ky:Lym,mzp:nz)=u1/sqrt(as(1,Lx,ky:Lym,mzp:nz))
   endif
   if(mby)then    ; xLb(myp:ny,kz:Lzm)=xLb(my:nyr:-1,kz:Lzm)
   elseif(Lbz)then; xLb(myp:ny,kz:Lzm)=xLb(myp:ny,kzr:Lz:-1)
   else           ; xLb(myp:ny,kz:Lzm)=u1/sqrt(as(1,Lx,myp:ny,kz:Lzm))
   endif
   if(mby)then    ; xLb(myp:ny,mzp:nz)=xLb(my:nyr:-1,mzp:nz)
   elseif(mbz)then; xLb(myp:ny,mzp:nz)=xLb(myp:ny,mz:nzr:-1)
   else           ; xLb(myp:ny,mzp:nz)=u1/sqrt(as(1,Lx,myp:ny,mzp:nz))
   endif
endif
...
if(mbx)then! "East":
   Lym=Ly-1; myp=my+1; ky=Ly-hy; ny=my+hy; kyr=Lym+hy; nyr=myp-hy
   Lzm=Lz-1; mzp=mz+1; kz=Lz-hz; nz=mz+hz; kzr=Lzm+hz; nzr=mzp-hz
   xmb(Ly:my,Lz:mz)=u1/sqrt(as(1,mx,Ly:my,Lz:mz))
   if(Lby)then; xmb(ky:Lym,Lz:mz)=xmb(kyr:Ly:-1,Lz:mz)
   else       ; xmb(ky:Lym,Lz:mz)=u1/sqrt(as(1,mx,ky:Lym,Lz:mz))
   endif
   if(mby)then; xmb(myp:ny,Lz:mz)=xmb(my:nyr:-1,Lz:mz)
   else       ; xmb(myp:ny,Lz:mz)=u1/sqrt(as(1,mx,myp:ny,Lz:mz))
   endif
   if(Lbz)then; xmb(Ly:my,kz:Lzm)=xmb(Ly:my,kzr:Lz:-1)
   else       ; xmb(Ly:my,kz:Lzm)=u1/sqrt(as(1,mx,Ly:my,kz:Lzm))
   endif
   if(mbz)then; xmb(Ly:my,mzp:nz)=xmb(Ly:my,mz:nzr:-1)
   else       ; xmb(Ly:my,mzp:nz)=u1/sqrt(as(1,mx,Ly:my,mzp:nz))
   endif
   if(Lby)then    ; xmb(ky:Lym,kz:Lzm)=xmb(kyr:Ly:-1,kz:Lzm)
   elseif(Lbz)then; xmb(ky:Lym,kz:Lzm)=xmb(ky:Lym,kzr:Lz:-1)
   else           ; xmb(ky:Lym,kz:Lzm)=u1/sqrt(as(1,mx,ky:Lym,kz:Lzm))
   endif
   if(Lby)then    ; xmb(ky:Lym,mzp:nz)=xmb(kyr:Ly:-1,mzp:nz)
   elseif(mbz)then; xmb(ky:Lym,mzp:nz)=xmb(ky:Lym,mz:nzr:-1)
   else           ; xmb(ky:Lym,mzp:nz)=u1/sqrt(as(1,mx,ky:Lym,mzp:nz))
   endif
   if(mby)then    ; xmb(myp:ny,kz:Lzm)=xmb(my:nyr:-1,kz:Lzm)
   elseif(Lbz)then; xmb(myp:ny,kz:Lzm)=xmb(myp:ny,kzr:Lz:-1)
   else           ; xmb(myp:ny,kz:Lzm)=u1/sqrt(as(1,mx,myp:ny,kz:Lzm))
   endif
   if(mby)then    ; xmb(myp:ny,mzp:nz)=xmb(my:nyr:-1,mzp:nz)
   elseif(mbz)then; xmb(myp:ny,mzp:nz)=xmb(myp:ny,mz:nzr:-1)
   else           ; xmb(myp:ny,mzp:nz)=u1/sqrt(as(1,mx,myp:ny,mzp:nz))
   endif
endif
...
if(Lby)then! "South":
   Lxm=Lx-1; mxp=mx+1; kx=Lx-hx; nx=mx+hx; kxr=Lxm+hx; nxr=mxp-hx
   Lzm=Lz-1; mzp=mz+1; kz=Lz-hz; nz=mz+hz; kzr=Lzm+hz; nzr=mzp-hz
   yLb(Lx:mx,Lz:mz)=u1/sqrt(as(2,Lx:mx,Ly,Lz:mz))
   if(Lbx)then; yLb(kx:Lxm,Lz:mz)=yLb(kxr:Lx:-1,Lz:mz)
   else       ; yLb(kx:Lxm,Lz:mz)=u1/sqrt(as(2,kx:Lxm,Ly,Lz:mz))
   endif
   if(mbx)then; yLb(mxp:nx,Lz:mz)=yLb(mx:nxr:-1,Lz:mz)
   else       ; yLb(mxp:nx,Lz:mz)=u1/sqrt(as(2,mxp:nx,Ly,Lz:mz))
   endif
   if(Lbz)then; yLb(Lx:mx,kz:Lzm)=yLb(Lx:mx,kzr:Lz:-1)
   else       ; yLb(Lx:mx,kz:Lzm)=u1/sqrt(as(2,Lx:mx,Ly,kz:Lzm))
   endif
   if(mbz)then; yLb(Lx:mx,mzp:nz)=yLb(Lx:mx,mz:nzr:-1)
   else       ; yLb(Lx:mx,mzp:nz)=u1/sqrt(as(2,Lx:mx,Ly,mzp:nz))
   endif
   if(Lbx)then    ; yLb(kx:Lxm,kz:Lzm)=yLb(kxr:Lx:-1,kz:Lzm)
   elseif(Lbz)then; yLb(kx:Lxm,kz:Lzm)=yLb(kx:Lxm,kzr:Lz:-1)
   else           ; yLb(kx:Lxm,kz:Lzm)=u1/sqrt(as(2,kx:Lxm,Ly,kz:Lzm))
   endif
   if(Lbx)then    ; yLb(kx:Lxm,mzp:nz)=yLb(kxr:Lx:-1,mzp:nz)
   elseif(mbz)then; yLb(kx:Lxm,mzp:nz)=yLb(kx:Lxm,mz:nzr:-1)
   else           ; yLb(kx:Lxm,mzp:nz)=u1/sqrt(as(2,kx:Lxm,Ly,mzp:nz))
   endif
   if(mbx)then    ; yLb(mxp:nx,kz:Lzm)=yLb(mx:nxr:-1,kz:Lzm)
   elseif(Lbz)then; yLb(mxp:nx,kz:Lzm)=yLb(mxp:nx,kzr:Lz:-1)
   else           ; yLb(mxp:nx,kz:Lzm)=u1/sqrt(as(2,mxp:nx,Ly,kz:Lzm))
   endif
   if(mbx)then    ; yLb(mxp:nx,mzp:nz)=yLb(mx:nxr:-1,mzp:nz)
   elseif(mbz)then; yLb(mxp:nx,mzp:nz)=yLb(mxp:nx,mz:nzr:-1)
   else           ; yLb(mxp:nx,mzp:nz)=u1/sqrt(as(2,mxp:nx,Ly,mzp:nz))
   endif
endif
...
if(mby)then! "North":
   Lxm=Lx-1; mxp=mx+1; kx=Lx-hx; nx=mx+hx; kxr=Lxm+hx; nxr=mxp-hx
   Lzm=Lz-1; mzp=mz+1; kz=Lz-hz; nz=mz+hz; kzr=Lzm+hz; nzr=mzp-hz
   ymb(Lx:mx,Lz:mz)=u1/sqrt(as(2,Lx:mx,my,Lz:mz))
   if(Lbx)then; ymb(kx:Lxm,Lz:mz)=ymb(kxr:Lx:-1,Lz:mz)
   else       ; ymb(kx:Lxm,Lz:mz)=u1/sqrt(as(2,kx:Lxm,my,Lz:mz))
   endif
   if(mbx)then; ymb(mxp:nx,Lz:mz)=ymb(mx:nxr:-1,Lz:mz)
   else       ; ymb(mxp:nx,Lz:mz)=u1/sqrt(as(2,mxp:nx,my,Lz:mz))
   endif
   if(Lbz)then; ymb(Lx:mx,kz:Lzm)=ymb(Lx:mx,kzr:Lz:-1)
   else       ; ymb(Lx:mx,kz:Lzm)=u1/sqrt(as(2,Lx:mx,my,kz:Lzm))
   endif
   if(mbz)then; ymb(Lx:mx,mzp:nz)=ymb(Lx:mx,mz:nzr:-1)
   else       ; ymb(Lx:mx,mzp:nz)=u1/sqrt(as(2,Lx:mx,my,mzp:nz))
   endif
   if(Lbx)then    ; ymb(kx:Lxm,kz:Lzm)=ymb(kxr:Lx:-1,kz:Lzm)
   elseif(Lbz)then; ymb(kx:Lxm,kz:Lzm)=ymb(kx:Lxm,kzr:Lz:-1)
   else           ; ymb(kx:Lxm,kz:Lzm)=u1/sqrt(as(2,kx:Lxm,my,kz:Lzm))
   endif
   if(Lbx)then    ; ymb(kx:Lxm,mzp:nz)=ymb(kxr:Lx:-1,mzp:nz)
   elseif(mbz)then; ymb(kx:Lxm,mzp:nz)=ymb(kx:Lxm,mz:nzr:-1)
   else           ; ymb(kx:Lxm,mzp:nz)=u1/sqrt(as(2,kx:Lxm,my,mzp:nz))
   endif
   if(mbx)then    ; ymb(mxp:nx,kz:Lzm)=ymb(mx:nxr:-1,kz:Lzm)
   elseif(Lbz)then; ymb(mxp:nx,kz:Lzm)=ymb(mxp:nx,kzr:Lz:-1)
   else           ; ymb(mxp:nx,kz:Lzm)=u1/sqrt(as(2,mxp:nx,my,kz:Lzm))
   endif
   if(mbx)then    ; ymb(mxp:nx,mzp:nz)=ymb(mx:nxr:-1,mzp:nz)
   elseif(mbz)then; ymb(mxp:nx,mzp:nz)=ymb(mxp:nx,mz:nzr:-1)
   else           ; ymb(mxp:nx,mzp:nz)=u1/sqrt(as(2,mxp:nx,my,mzp:nz))
   endif
endif
...
if(Lbz)then! "Bottom":
   Lxm=Lx-1; mxp=mx+1; kx=Lx-hx; nx=mx+hx; kxr=Lxm+hx; nxr=mxp-hx
   Lym=Ly-1; myp=my+1; ky=Ly-hy; ny=my+hy; kyr=Lym+hy; nyr=myp-hy
   zLb(Lx:mx,Ly:my)=u1/sqrt(as(3,Lx:mx,Ly:my,Lz))
   if(Lbx)then; zLb(kx:Lxm,Ly:my)=zLb(kxr:Lx:-1,Ly:my)
   else       ; zLb(kx:Lxm,Ly:my)=u1/sqrt(as(3,kx:Lxm,Ly:my,Lz))
   endif
   if(mbx)then; zLb(mxp:nx,Ly:my)=zLb(mx:nxr:-1,Ly:my)
   else       ; zLb(mxp:nx,Ly:my)=u1/sqrt(as(3,mxp:nx,Ly:my,Lz))
   endif
   if(Lby)then; zLb(Lx:mx,ky:Lym)=zLb(Lx:mx,kyr:Ly:-1)
   else       ; zLb(Lx:mx,ky:Lym)=u1/sqrt(as(3,Lx:mx,ky:Lym,Lz))
   endif
   if(mby)then; zLb(Lx:mx,myp:ny)=zLb(Lx:mx,my:nyr:-1)
   else       ; zLb(Lx:mx,myp:ny)=u1/sqrt(as(3,Lx:mx,myp:ny,Lz))
   endif
   if(Lbx)then    ; zLb(kx:Lxm,ky:Lym)=zLb(kxr:Lx:-1,ky:Lym)
   elseif(Lby)then; zLb(kx:Lxm,ky:Lym)=zLb(kx:Lxm,kyr:Ly:-1)
   else           ; zLb(kx:Lxm,ky:Lym)=u1/sqrt(as(3,kx:Lxm,ky:Lym,Lz))
   endif
   if(Lbx)then    ; zLb(kx:Lxm,myp:ny)=zLb(kxr:Lx:-1,myp:ny)
   elseif(mby)then; zLb(kx:Lxm,myp:ny)=zLb(kx:Lxm,my:nyr:-1)
   else           ; zLb(kx:Lxm,myp:ny)=u1/sqrt(as(3,kx:Lxm,myp:ny,Lz))
   endif
   if(mbx)then    ; zLb(mxp:nx,ky:Lym)=zLb(mx:nxr:-1,ky:Lym)
   elseif(Lby)then; zLb(mxp:nx,ky:Lym)=zLb(mxp:nx,kyr:Ly:-1)
   else           ; zLb(mxp:nx,ky:Lym)=u1/sqrt(as(3,mxp:nx,ky:Lym,Lz))
   endif
   if(mbx)then    ; zLb(mxp:nx,myp:ny)=zLb(mx:nxr:-1,myp:ny)
   elseif(mby)then; zLb(mxp:nx,myp:ny)=zLb(mxp:nx,my:nyr:-1)
   else           ; zLb(mxp:nx,myp:ny)=u1/sqrt(as(3,mxp:nx,myp:ny,Lz))
   endif
endif
...
if(mbz)then! "Top":
   Lxm=Lx-1; mxp=mx+1; kx=Lx-hx; nx=mx+hx; kxr=Lxm+hx; nxr=mxp-hx
   Lym=Ly-1; myp=my+1; ky=Ly-hy; ny=my+hy; kyr=Lym+hy; nyr=myp-hy
   zmb(Lx:mx,Ly:my)=u1/sqrt(as(3,Lx:mx,Ly:my,mz))
   if(Lbx)then; zmb(kx:Lxm,Ly:my)=zmb(kxr:Lx:-1,Ly:my)
   else       ; zmb(kx:Lxm,Ly:my)=u1/sqrt(as(3,kx:Lxm,Ly:my,mz))
   endif
   if(mbx)then; zmb(mxp:nx,Ly:my)=zmb(mx:nxr:-1,Ly:my)
   else       ; zmb(mxp:nx,Ly:my)=u1/sqrt(as(3,mxp:nx,Ly:my,mz))
   endif
   if(Lby)then; zmb(Lx:mx,ky:Lym)=zmb(Lx:mx,kyr:Ly:-1)
   else       ; zmb(Lx:mx,ky:Lym)=u1/sqrt(as(3,Lx:mx,ky:Lym,mz))
   endif
   if(mby)then; zmb(Lx:mx,myp:ny)=zmb(Lx:mx,my:nyr:-1)
   else       ; zmb(Lx:mx,myp:ny)=u1/sqrt(as(3,Lx:mx,myp:ny,mz))
   endif
   if(Lbx)then    ; zmb(kx:Lxm,ky:Lym)=zmb(kxr:Lx:-1,ky:Lym)
   elseif(Lby)then; zmb(kx:Lxm,ky:Lym)=zmb(kx:Lxm,kyr:Ly:-1)
   else           ; zmb(kx:Lxm,ky:Lym)=u1/sqrt(as(3,kx:Lxm,ky:Lym,mz))
   endif
   if(Lbx)then    ; zmb(kx:Lxm,myp:ny)=zmb(kxr:Lx:-1,myp:ny)
   elseif(mby)then; zmb(kx:Lxm,myp:ny)=zmb(kx:Lxm,my:nyr:-1)
   else           ; zmb(kx:Lxm,myp:ny)=u1/sqrt(as(3,kx:Lxm,myp:ny,mz))
   endif
   if(mbx)then    ; zmb(mxp:nx,ky:Lym)=zmb(mx:nxr:-1,ky:Lym)
   elseif(Lby)then; zmb(mxp:nx,ky:Lym)=zmb(mxp:nx,kyr:Ly:-1)
   else           ; zmb(mxp:nx,ky:Lym)=u1/sqrt(as(3,mxp:nx,ky:Lym,mz))
   endif
   if(mbx)then    ; zmb(mxp:nx,myp:ny)=zmb(mx:nxr:-1,myp:ny)
   elseif(mby)then; zmb(mxp:nx,myp:ny)=zmb(mxp:nx,my:nyr:-1)
   else           ; zmb(mxp:nx,myp:ny)=u1/sqrt(as(3,mxp:nx,myp:ny,mz))
   endif
endif
...
do iz=Lz,mz; do iy=Ly,my; do ix=Lx,mx
   ...
   if(Lbx)then
      ...
      gxn=gxm+Lxmix
      do gx=1,gxn
         rx(-gym:gym,-gzm:gzm)=gx*xLb(iy-gym:iy+gym,iz-gzm:iz+gzm)
         do gz=-gzm,gzm; do gy=-gym,gym; rx(gy,gz)=om0+rx(gy,gz)*(om1+rx(gy,gz)*om2) 
         enddo; enddo
         ixm=lxmix-gx
         ixp=lxmix+gx-1
         fs(ixp,-gym:gym,-gzm:gzm)=fs(ixp,-gym:gym,-gzm:gzm)+rx(-gym:gym,-gzm:gzm)&
              *fs(ixm,-gym:gym,-gzm:gzm); fs(ixm,-gym:gym,-gzm:gzm)=0
      enddo
   endif
   if(mbx)then
      mxmix=mx-ix
      gxn=gxm-mxmix
      do gx=1,gxn
         rx(-gym:gym,-gzm:gzm)=gx*xmb(iy-gym:iy+gym,iz-gzm:iz+gzm)
         do gz=-gzm,gzm; do gy=-gym,gym; rx(gy,gz)=om0+rx(gy,gz)*(om1+rx(gy,gz)*om2)
         enddo; enddo
         ixp=mxmix+gx
         ixm=mxmix-gx+1
         fs(ixm,-gym:gym,-gzm:gzm)=fs(ixm,-gym:gym,-gzm:gzm)+rx(-gym:gym,-gzm:gzm)&
              *fs(ixp,-gym:gym,-gzm:gzm); fs(ixp,-gym:gym,-gzm:gzm)=0
      enddo
   endif
   ...
   if(Lby)then
      ...
      gyn=gym+Lymiy
      do gy=1,gyn
         ry(-gxm:gxm,-gzm:gzm)=gy*yLb(ix-gxm:ix+gxm,iz-gzm:iz+gzm)
         do gz=-gzm,gzm; do gx=-gxm,gxm; ry(gx,gz)=om0+ry(gx,gz)*(om1+ry(gx,gz)*om2)
         enddo; enddo
         iym=Lymiy-gy
         iyp=Lymiy+gy-1
         fs(-gxm:gxm,iyp,-gzm:gzm)=fs(-gxm:gxm,iyp,-gzm:gzm)+ry(-gxm:gxm,-gzm:gzm)&
              *fs(-gxm:gxm,iym,-gzm:gzm); fs(-gxm:gxm,iym,-gzm:gzm)=0
      enddo
   endif
   if(mby)then
      mymiy=my-iy
      gyn=gym-mymiy
      do gy=1,gyn
         ry(-gxm:gxm,-gzm:gzm)=gy*ymb(ix-gxm:ix+gxm,iz-gzm:iz+gzm)
         do gz=-gzm,gzm; do gx=-gxm,gxm; ry(gx,gz)=om0+ry(gx,gz)*(om1+ry(gx,gz)*om2)
         enddo; enddo
         iyp=mymiy+gy
         iym=mymiy-gy+1
         fs(-gxm:gxm,iym,-gzm:gzm)=fs(-gxm:gxm,iym,-gzm:gzm)+ry(-gxm:gxm,-gzm:gzm)&
              *fs(-gxm:gxm,iyp,-gzm:gzm); fs(-gxm:gxm,iyp,-gzm:gzm)=0
      enddo
   endif
   ...
   if(Lbz)then
      ...
      gzn=gzm+Lzmiz
      do gz=1,gzn
         rz(-gxm:gxm,-gym:gym)=gz*zLb(ix-gxm:ix+gxm,iy-gym:iy+gym)
         do gy=-gym,gym; do gx=-gxm,gxm; rz(gx,gy)=om0+rz(gx,gy)*(om1+rz(gx,gy)*om2)
         enddo; enddo
         izm=Lzmiz-gz
         izp=Lzmiz+gz-1
         fs(-gxm:gxm,-gym:gym,izp)=fs(-gxm:gxm,-gym:gym,izp)+rz(-gxm:gxm,-gym:gym)&
              *fs(-gxm:gxm,-gym:gym,izm); fs(-gxm:gxm,-gym:gym,izm)=0
      enddo
   endif
   if(mbz)then
      mzmiz=mz-iz
      gzn=gzm-mzmiz
      do gz=1,gzn
         rz(-gxm:gxm,-gym:gym)=gz*zmb(ix-gxm:ix+gxm,iy-gym:iy+gym)
         do gy=-gym,gym; do gx=-gxm,gxm; rz(gx,gy)=om0+rz(gx,gy)*(om1+rz(gx,gy)*om2)
         enddo; enddo
         izp=mzmiz+gz
         izm=mzmiz-gz+1
         fs(-gxm:gxm,-gym:gym,izm)=fs(-gxm:gxm,-gym:gym,izm)+rz(-gxm:gxm,-gym:gym)&
              *fs(-gxm:gxm,-gym:gym,izp); fs(-gxm:gxm,-gym:gym,izp)=0
      enddo
   endif
...
if(Lb)then; Lxm=Lx-1! reflect exterior halo <Lx inwards and add; set the halo to 0
   do gx=1,hx; ixm=Lx-gx; ixp=Lxm+gx; rx=gx*xLb; rx=om0+rx*(om1+rx*om2)
      a(ixp)=a(ixp)+rx*a(ixm); a(ixm)=0
   enddo
endif
if(mb)then; mxp=mx+1! reflect exterior halo >mx inwards and add:
   do gx=1,hx; ixp=mx+gx; ixm=mxp-gx; rx=gx*xmb; rx=om0+rx*(om1+rx*om2)
      a(ixm)=a(ixm)+rx*a(ixp); a(ixp)=0
   enddo
endif
...
integer(spi),intent(in   ):: hx,Lx,mx,hy,Ly,my
logical,     intent(in   ):: Lbx,mbx,Lby,mby
real(dp),dimension(             -hy+Ly:my+hy),intent(in   ):: xLb,xmb
real(dp),dimension(-hx+Lx:mx+hx             ),intent(in   ):: yLb,ymb
real(dp),dimension(-hx+Lx:mx+hx,-hy+Ly:my+hy),intent(inout):: a
!------------------------------------------------------------------------------
real(dp),dimension(-hy+Ly:my+hy):: rx
real(dp),dimension(-hx+Lx:mx+hx):: ry
...
if(Lby)then; Lym=Ly-1! reflect exterior halo <Ly inwards and add:
   do gy=1,hy; iym=Ly-gy; iyp=Lym+gy; ry=gy*yLb; ry=om0+ry*(om1+ry*om2)
      a(kx:nx,iyp)=a(kx:nx,iyp)+ry*a(kx:nx,iym); a(kx:nx,iym)=0
   enddo!       gy
endif
if(mby)then; myp=my+1! reflect exterior halo >my inwards and add:
   do gy=1,hy; iyp=my+gy; iym=myp-gy; ry=gy*ymb; ry=om0+ry*(om1+ry*om2)
      a(kx:nx,iym)=a(kx:nx,iym)+ry*a(kx:nx,iyp); a(kx:nx,iyp)=0
   enddo!       gy
endif

if(Lbx)then; Lxm=Lx-1! reflect exterior halo <Lx inwards and add:
   do gx=1,hx; ixm=Lx-gx; ixp=Lxm+gx; rx=gx*xLb; rx=om0+rx*(om1+rx*om2)
      a(ixp,ky:ny)=a(ixp,ky:ny)+rx*a(ixm,ky:ny); a(ixm,ky:ny)=0
   enddo!       gx
endif
if(mbx)then; mxp=mx+1! reflect exterior halo >mx inwards and add:
   do gx=1,hx; ixp=mx+gx; ixm=mxp-gx; rx=gx*xmb; rx=om0+rx*(om1+rx*om2)
      a(ixm,ky:ny)=a(ixm,ky:ny)+rx*a(ixp,ky:ny); a(ixp,ky:ny)=0
   enddo!       gx
endif
...
real(dp),dimension(-hy+Ly:my+hy):: rx
real(dp),dimension(-hx+Lx:mx+hx):: ry
integer(spi):: gx,gy,ixm,ixp,iym,iyp,kx,ky,Lxm,mxp,Lym,myp,nx,ny,iv
!==============================================================================
kx=Lx-hx; nx=mx+hx; ky=Ly-hy; ny=my+hy

if(Lby)then; Lym=Ly-1! reflect exterior halo <Ly inwards and add:
   do gy=1,hy; iym=Ly-gy; iyp=Lym+gy; ry=gy*yLb; ry=om0+ry*(om1+ry*om2)
      do iv=1,nv; a(iv,kx:nx,iyp)=a(iv,kx:nx,iyp)+a(iv,kx:nx,iym)*ry; a(iv,kx:nx,iym)=0
      enddo! iv
   enddo!       gy
endif
if(mby)then; myp=my+1! reflect exterior halo >my inwards and add:
   do gy=1,hy; iyp=my+gy; iym=myp-gy; ry=gy*ymb; ry=om0+ry*(om1+ry*om2)
      do iv=1,nv; a(iv,kx:nx,iym)=a(iv,kx:nx,iym)+a(iv,kx:nx,iyp)*ry; a(iv,kx:nx,iyp)=0
      enddo! iv
   enddo!       gy
endif
...
if(Lbx)then; Lxm=Lx-1! reflect exterior halo <Lx inwards and add:
   do gx=1,hx; ixm=Lx-gx; ixp=Lxm+gx; rx=gx*xLb; rx=om0+rx*(om1+rx*om2)
      do iv=1,nv; a(iv,ixp,ky:ny)=a(iv,ixp,ky:ny)+a(iv,ixm,ky:ny)*rx; a(iv,ixm,ky:ny)=0
      enddo! iv
   enddo!       gx
endif
if(mbx)then; mxp=mx+1! reflect exterior halo >mx inwards and add:
   do gx=1,hx; ixp=mx+gx; ixm=mxp-gx; rx=gx*xmb; rx=om0+rx*(om1+rx*om2)
      do iv=1,nv; a(iv,ixm,ky:ny)=a(iv,ixm,ky:ny)+a(iv,ixp,ky:ny)*rx; a(iv,ixp,ky:ny)=0
      enddo! iv
   enddo!       gx
endif
...
integer(spi),intent(in   ):: hx,Lx,mx, hy,ly,my, hz,lz,mz
logical,     intent(in   ):: Lbx,mbx,Lby,mby,Lbz,mbz
real(dp),dimension(             -hy+Ly:my+hy,-hz+Lz:mz+hz),intent(in   ):: xLb,xmb
real(dp),dimension(-hx+Lx:mx+hx             ,-hz+Lz:mz+hz),intent(in   ):: yLb,ymb
real(dp),dimension(-hx+Lx:mx+hx,-hx+Lx:mx+hx             ),intent(in   ):: zLb,zmb
real(dp),dimension(-hx+Lx:mx+hx,-hy+Ly:my+hy,-hz+Lz:mz+hz),intent(inout):: a
!----------------------------------------------------------------------------------
real(dp),dimension(             -hy+Ly:my+hy,-hz+Lz:mz+hz):: rx
real(dp),dimension(-hx+Lx:mx+hx,             -hz+Lz:mz+hz):: ry
real(dp),dimension(-hx+Lx:mx+hx,-hy+Ly:my+hy             ):: rz
...
if(Lbz)then; Lzm=Lz-1! reflect exterior halo <Lz inwards and add:
   do gz=1,hz; izm=Lz-gz; izp=Lzm+gz; rz=gz*zLb; rz=om0+rz*(om1+rz*om2)
      a(kx:nx,ky:ny,izp)=a(kx:nx,ky:ny,izp)+rz*a(kx:nx,ky:ny,izm); a(kx:nx,ky:ny,izm)=0
   enddo!            gz
endif
if(mbz)then; mzp=mz+1! reflect exterior halo >mz inwards and add:
   do gz=1,hz; izp=mz+gz; izm=mzp-gz; rz=gz*zmb; rz=om0+rz*(om1+rz*om2)
      a(kx:nx,ky:ny,izm)=a(kx:nx,ky:ny,izm)+rz*a(kx:nx,ky:ny,izp); a(kx:nx,ky:ny,izp)=0
   enddo!           gz
endif

if(Lby)then; Lym=Ly-1! reflect exterior halo <Ly inwards and add:
   do gy=1,hy; iym=Ly-gy; iyp=Lym+gy; ry=gy*yLb; ry=om0+ry*(om1+ry*om2)
      a(kx:nx,iyp,kz:nz)=a(kx:nx,iyp,kz:nz)+ry*a(kx:nx,iym,kz:nz); a(kx:nx,iym,kz:nz)=0
   enddo!        gy
endif
if(mby)then; myp=my+1! reflect exterior halo >my inwards and add:
   do gy=1,hy; iyp=my+gy; iym=myp-gy; ry=gy*ymb; ry=om0+ry*(om1+ry*om2)
      a(kx:nx,iym,kz:nz)=a(kx:nx,iym,kz:nz)+ry*a(kx:nx,iyp,kz:nz); a(kx:nx,iyp,kz:nz)=0
   enddo!        gy
endif

if(Lbx)then; Lxm=Lx-1! reflect exterior halo <Lx inwards and add:
   do gx=1,hx; ixm=Lx-gx; ixp=Lxm+gx; rx=gx*xLb; rx=om0+rx*(om1+rx*om2)
      a(ixp,ky:ny,kz:nz)=a(ixp,ky:ny,kz:nz)+rx*a(ixm,ky:ny,kz:nz); a(ixm,ky:ny,kz:nz)=0
   enddo!     gx
endif
if(mbx)then; mxp=mx+1! reflect exterior halo >mx inwards and add:
   do gx=1,hx; ixp=mx+gx; ixm=mxp-gx; rx=gx*xmb; rx=om0+rx*(om1+rx*om2)
      a(ixm,ky:ny,kz:nz)=a(ixm,ky:ny,kz:nz)+rx*a(ixp,ky:ny,kz:nz); a(ixp,ky:ny,kz:nz)=0
   enddo!     gx
endif
...
real(dp),dimension(-hx+Lx:mx+hx,-hy+Ly:my+hy             ):: rz
integer(spi):: gx,gy,gz,ixp,ixm,iyp,iym,izp,izm,kx,ky,kz, &
               Lxm,mxp,Lym,myp,Lzm,mzp,nx,ny,nz,iv
!==================================================================================
kx=Lx-hx; nx=mx+hx; ky=Ly-hy; ny=my+hy; kz=Lz-hz; nz=mz+hz

if(Lbz)then; Lzm=Lz-1! reflect exterior halo <Lz inwards and add:
   do gz=1,hz; izm=Lz-gz; izp=Lzm+gz; rz=gz*zLb; rz=om0+rz*(om1+rz*om2)
      do iv=1,nv;a(iv,kx:nx,ky:ny,izp)=a(iv,kx:nx,ky:ny,izp)+rz*a(iv,kx:nx,ky:ny,izm)
         a(iv,kx:nx,ky:ny,izm)=0
      enddo! iv
   enddo!            gz
endif
if(mbz)then; mzp=mz+1! reflect exterior halo >mz inwards and add:
   do gz=1,hz; izp=mz+gz; izm=mzp-gz; rz=gz*zmb; rz=om0+rz*(om1+rz*om2)
      do iv=1,nv;a(iv,kx:nx,ky:ny,izm)=a(iv,kx:nx,ky:ny,izm)+rz*a(iv,kx:nx,ky:ny,izp)
         a(iv,kx:nx,ky:ny,izp)=0
      enddo! iv
   enddo!           gz
endif
...
if(Lby)then; Lym=Ly-1! reflect exterior halo <Ly inwards and add:
   do gy=1,hy; iym=Ly-gy; iyp=Lym+gy; ry=gy*yLb; ry=om0+ry*(om1+ry*om2)
      do iv=1,nv;a(iv,kx:nx,iyp,kz:nz)=a(iv,kx:nx,iyp,kz:nz)+ry*a(iv,kx:nx,iym,kz:nz)
         a(iv,kx:nx,iym,kz:nz)=0
      enddo! iv
   enddo!        gy
endif
if(mby)then; myp=my+1! reflect exterior halo >my inwards and add:
   do gy=1,hy; iyp=my+gy; iym=myp-gy; ry=gy*ymb; ry=om0+ry*(om1+ry*om2)
      do iv=1,nv;a(iv,kx:nx,iym,kz:nz)=a(iv,kx:nx,iym,kz:nz)+ry*a(iv,kx:nx,iyp,kz:nz)
         a(iv,kx:nx,iyp,kz:nz)=0
      enddo! iv
   enddo!        gy
endif
...
if(Lbx)then; Lxm=Lx-1! reflect exterior halo <Lx inwards and add:
   do gx=1,hx; ixm=Lx-gx; ixp=Lxm+gx; rx=gx*xLb; rx=om0+rx*(om1+rx*om2)
      do iv=1,nv;a(iv,ixp,ky:ny,kz:nz)=a(iv,ixp,ky:ny,kz:nz)+rx*a(iv,ixm,ky:ny,kz:nz)
         a(iv,ixm,ky:ny,kz:nz)=0
      enddo! iv
   enddo!     gx
endif
if(mbx)then; mxp=mx+1! reflect exterior halo >mx inwards and add:
   do gx=1,hx; ixp=mx+gx; ixm=mxp-gx; rx=gx*xmb; rx=om0+rx*(om1+rx*om2)
      do iv=1,nv;a(iv,ixm,ky:ny,kz:nz)=a(iv,ixm,ky:ny,kz:nz)+rx*a(iv,ixp,ky:ny,kz:nz)
         a(iv,ixp,ky:ny,kz:nz)=0
      enddo! iv
   enddo!     gx
endif
...
real(dp),dimension(-hy+Ly:my+hy):: rx
real(dp),dimension(-hx+Lx:mx+hx):: ry
integer(spi):: gx,gy,ixm,ixp,iym,iyp,kx,ky,Lxm,Lym,mxp,myp,nx,ny,iv
!==============================================================================
kx=Lx-hx; nx=mx+hx; ky=Ly-hy; ny=my+hy

if(Lbx)then; Lxm=Lx-1! reflect values into the exterior halo <Lx:
   do gx=1,hx; ixm=Lx-gx; ixp=Lxm+gx; rx=gx*xlb; rx=om0+rx*(om1+rx*om2)
      do iv=1,nv; a(iv,ixm,ky:ny)=a(iv,ixp,ky:ny)*rx
      enddo! iv
   enddo! gx
endif
if(mbx)then; mxp=mx+1! reflect values into the exterior halo >mx:
   do gx=1,hx; ixp=mx+gx; ixm=mxp-gx; rx=gx*xmb; rx=om0+rx*(om1+rx*om2)
      do iv=1,nv; a(iv,ixp,ky:ny)=a(iv,ixm,ky:ny)*rx
      enddo! iv
   enddo! gx
endif
...
if(Lby)then; Lym=Ly-1! reflect values into the exterior halo <Ly:
   do gy=1,hy; iym=Ly-gy; iyp=Lym+gy; ry=gy*yLb; ry=om0+ry*(om1+ry*om2)
      do iv=1,nv; a(iv,kx:nx,iym)=a(iv,kx:nx,iyp)*ry
      enddo! iv
   enddo!   gy
endif
if(mby)then; myp=my+1! reflect values into the exterior halo >my:
   do gy=1,hy; iyp=my+gy; iym=myp-gy; ry=gy*ymb; ry=om0+ry*(om1+ry*om2)
      do iv=1,nv; a(iv,kx:nx,iyp)=a(iv,kx:nx,iym)*ry
      enddo! iv
   enddo!   gy
endif
...
real(dp),dimension(             -hy+Ly:my+hy,-hz+Lz:mz+hz),intent(in   ):: xLb,xmb
real(dp),dimension(-hx+Lx:mx+hx,             -hz+Lz:mz+hz),intent(in   ):: yLb,ymb
real(dp),dimension(-hx+Lx:mx+hx,-hy+Ly:my+hy             ),intent(in   ):: zLb,zmb
real(dp),dimension(-hx+Lx:mx+hx,-hy+Ly:my+hy,-hz+Lz:mz+hz),intent(inout):: a
!----------------------------------------------------------------------------------
real(dp),dimension(             -hy+Ly:my+hy,-hz+Lz:mz+hz):: rx
real(dp),dimension(-hx+Lx:mx+hx,             -hz+Lz:mz+hz):: ry
...
if(Lbx)then; Lxm=Lx-1! reflect values into the exterior halo <Lx:
   do gx=1,hx; ixm=Lx-gx; ixp=Lxm+gx; rx=gx*xLb; rx=om0+rx*(om1+rx*om2)
      a(ixm,ky:ny,kz:nz)=rx*a(ixp,ky:ny,kz:nz)
   enddo!    gx
endif
if(mbx)then; mxp=mx+1! reflect values into the exterior halo >mx:
   do gx=1,hx; ixp=mx+gx; ixm=mxp-gx; rx=gx*xmb; rx=om0+rx*(om1+rx*om2)
      a(ixp,ky:ny,kz:nz)=rx*a(ixm,ky:ny,kz:nz)
   enddo!    gx
endif

if(Lby)then; Lym=Ly-1! reflect values into the exterior halo <Ly:
   do gy=1,hy; iym=Ly-gy; iyp=Lym+gy; ry=gy*yLb; ry=om0+ry*(om1+ry*om2)
      a(kx:nx,iym,kz:nz)=ry*a(kx:nx,iyp,kz:nz)
   enddo!        gy
endif
if(mby)then; myp=my+1! reflect values into the exterior halo >my:
   do gy=1,hy; iyp=my+gy; iym=myp-gy; ry=gy*ymb; ry=om0+ry*(om1+ry*om2)
      a(kx:nx,iyp,kz:nz)=ry*a(kx:nx,iym,kz:nz)
   enddo!        gy
endif

if(Lbz)then; Lzm=Lz-1! reflect interior values into the exterior halo <Lz:
   do gz=1,hz; izm=Lz-gz; izp=Lzm+gz; rz=gz*zLb; rz=om0+rz*(om1+rz*om2)
      a(kx:nx,ky:ny,izm)=rz*a(kx:nx,ky:ny,izp)
   enddo!            gz
endif
if(mbz)then; mzp=mz+1! reflect interior values into the exterior halo >mz:
   do gz=1,hz; izp=mz+gz; izm=mzp-gz; rz=gz*zmb; rz=om0+rz*(om1+rz*om2)
      a(kx:nx,ky:ny,izp)=rz*a(kx:nx,ky:ny,izm)
   enddo!           gz
endif
...
real(dp),dimension(-hx+Lx:mx+hx,-hy+Ly:my+hy             ):: rz
integer(spi):: gx,gy,gz,ixp,ixm,iyp,iym,izp,izm,&
            kx,ky,kz,Lxm,mxp,Lym,myp,Lzm,mzp,nx,ny,nz,iv
!==================================================================================
kx=Lx-hx; nx=mx+hx; ky=Ly-hy; ny=my+hy; kz=Lz-hz; nz=mz+hz

if(Lbx)then; Lxm=Lx-1! reflect values into the exterior halo <Lx:
   do gx=1,hx; ixm=Lx-gx; ixp=Lxm+gx; rx=gx*xLb; rx=om0+rx*(om1+rx*om2)
      do iv=1,nv; a(iv,ixm,ky:ny,kz:nz)=rx*a(iv,ixp,ky:ny,kz:nz)
      enddo! iv
   enddo!    gx
endif
if(mbx)then; mxp=mx+1! reflect values into the exterior halo >mx:
   do gx=1,hx; ixp=mx+gx; ixm=mxp-gx; rx=gx*xmb; rx=om0+rx*(om1+rx*om2)
      do iv=1,nv; a(iv,ixp,ky:ny,kz:nz)=rx*a(iv,ixm,ky:ny,kz:nz)
      enddo! iv
   enddo!    gx
endif
...
if(Lby)then; Lym=Ly-1! reflect values into the exterior halo <Ly:
   do gy=1,hy; iym=Ly-gy; iyp=Lym+gy; ry=gy*yLb; ry=om0+ry*(om1+ry*om2)
      do iv=1,nv; a(iv,kx:nx,iym,kz:nz)=ry*a(iv,kx:nx,iyp,kz:nz)
      enddo! iv
   enddo!        gy
endif
if(mby)then; myp=my+1! reflect values into the exterior halo >my:
   do gy=1,hy; iyp=my+gy; iym=myp-gy; ry=gy*ymb; ry=om0+ry*(om1+ry*om2)
      do iv=1,nv; a(iv,kx:nx,iyp,kz:nz)=ry*a(iv,kx:nx,iym,kz:nz)
      enddo! iv
   enddo!        gy
endif
...
if(Lbz)then; Lzm=Lz-1! reflect interior values into the exterior halo <Lz:
   do gz=1,hz; izm=Lz-gz; izp=Lzm+gz; rz=gz*zLb; rz=om0+rz*(om1+rz*om2)
      do iv=1,nv; a(iv,kx:nx,ky:ny,izm)=rz*a(iv,kx:nx,ky:ny,izp)
      enddo! iv
   enddo!            gz
endif
if(mbz)then; mzp=mz+1! reflect interior values into the exterior halo >mz:
   do gz=1,hz; izp=mz+gz; izm=mzp-gz; rz=gz*zmb; rz=om0+rz*(om1+rz*om2)
      do iv=1,nv; a(iv,kx:nx,ky:ny,izp)=rz*a(iv,kx:nx,ky:ny,izm)
      enddo! iv
   enddo!           gz
endif
